...
- `addChargeLine(...)` adds individual charge lines. Required setters are `chargeCode` and `description`; optional setters are `costAmount(value, currencyCode)`, `sellAmount(value, currencyCode)`, `costLocalAmount(value)`, `sellLocalAmount(value)`, `chargeCodeGroup`, `branch`, `department`, `debtor`, and `displaySequence`. Currency-bearing cost and sell amounts populate `CostOSAmount` / `SellOSAmount` and their OS currencies; Cord does not also copy them into local amounts. Use the local setters for currency-less local figures when no quote-level `goodsValue` currency is acting as the line default. Only the fields you provide appear in the outgoing XML — no rating behaviours, exchange rates, or posted flags are hard-coded. CargoWise eAdapter requires an `ImportMetaData` element on every charge line when importing; Cord adds `<ImportMetaData><Instruction>INSERT</Instruction></ImportMetaData>` automatically.
