...
- For one-off quote charge lines, an amount with a currency is an OS amount and populates `CostOSAmount` or `SellOSAmount`; Cord does not also populate the corresponding local amount. An amount without a currency is treated as local unless the quote-level `goods_value` currency supplies the line default. Use `costLocalAmount()` or `sellLocalAmount()` when a fluent caller needs to provide a local figure explicitly.
